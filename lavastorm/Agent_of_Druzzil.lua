function event_say(e)
	...
	elseif ( e.message:findi("nagafen") e.other:GetLevel() >= 30 and e.other:CastToClient():GuildID() ~= 0xFFFFFFFF ) then
	...
	elseif(e.message:findi("hail") e.other:GetLevel() >= 30 and e.other:CastToClient():GuildID() == 0xFFFFFFFF) then
		e.self:Say("Hail, mortal. You are lacking friends. Please gather some, then come back and approach me.");
	elseif(e.message:findi("hail")) then
		e.self:Say("Hail, mortal. You are too inexperienced to enter here.");
	end
end