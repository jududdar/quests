...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 2388}, 0)) then -- Qeynos Badge of Honor
		if(e.other:GetFaction(e.self) < 5) then -- eats item if not amiable or better
			...
			e.other:QuestReward(e.self,0,0,0,0,8285); -- Marked Qeynos Badge of Honor, official warning
			e.other:QuestReward(e.self,0,0,0,0,8283);
		end
	...
	elseif(e.other:GetFaction(e.self) < 5 and item_lib.check_turn_in(e.self, e.trade, {item1 = 8287}, 0)) then -- Orders for Sergeant Caelin
		...
		e.other:QuestReward(e.self,0,0,0,0,8282); -- writ of execution, Black Satchel
		e.other:QuestReward(e.self,0,0,0,0,17160);
